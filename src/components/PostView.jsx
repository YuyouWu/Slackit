...
class PostView extends React.Component {
  ...
  getPostData = () => {
    axios
      .get(`https://www.reddit.com${this.props.permaLink}.json`)
      .then((res) => {
        this.setState({
          loading: false,
  ...
  renderPostVideo = () => {
    if (
      this.state.postData["secure_media"] &&
      this.state.postData["secure_media"]["reddit_video"] &&
      this.state.postData["secure_media"]["reddit_video"]["hls_url"]
    ) {
      return (
      ...
      this.state.postData["secure_media"] &&
      this.state.postData["secure_media"].type === "youtube.com"
    ) {
      return (
        <ReactPlayer
  ...
  renderGIF = () => {
    if (
      this.state.postData.media &&
      this.state.postData.media.oembed &&
      this.state.postData.media.oembed["thumbnail_url"]
    ) {
      if (
        this.state.postData.media.oembed["provider_name"] !== "YouTube" &&
        !this.state.postData.url.endsWith("gifv")
      ) {
        return (
          <Image
    ...
    if (this.state.postData.url && this.state.postData.url.endsWith("gifv")) {
      if (
        this.state.postData.preview &&
        this.state.postData.preview["reddit_video_preview"]
      ) {
        return (
          <ReactPlayer
            url={
